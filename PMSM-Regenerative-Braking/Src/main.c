...
int main(void)
{
	...
	while (1)
	{
		...
		if (1 == M_Brake_u8 && MCI_GetAvrgMecSpeed_F(pMCI[0]) <= LOWEST_SPEED)
		{
			MC_StopMotor1();
		}
...
{
	float M_MotorTorque_f = 0.0;
	float M_BrakingTorque_f = 0.0;

	if (TIM6 == htim->Instance)
	{
		//MC_ProgramSpeedRampMotor1(0, 500);
		M_MotorTorque_f = MCI_GetIqdref_F(pMCI[0]).q;
		M_BrakingTorque_f = -M_MotorTorque_f;
		MC_ProgramTorqueRampMotor1_F(M_BrakingTorque_f, 0);
		M_Brake_u8 = 1;
	}
}
/* USER CODE END 4 */
